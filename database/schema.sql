-- AI 輔助部落文化旅遊推薦平台 資料庫設計
-- 對應 A11（景點資料表主鍵）、B2（外鍵關聯）
...
CREATE TABLE IF NOT EXISTS attractions (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    name        TEXT NOT NULL,          -- 部落／景點名稱
    city        TEXT NOT NULL,          -- 縣市或鄉鎮
    category_id INTEGER NOT NULL,       -- 族群分類（外鍵）
...
CREATE TABLE IF NOT EXISTS itineraries (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    name        TEXT NOT NULL,          -- 遊程名稱
    region      TEXT NOT NULL,          -- 區域（北部／中部／南部／東部）
...
CREATE TABLE IF NOT EXISTS reviews (
    ...
    rating_scenery INTEGER NOT NULL,  -- 景觀環境
    rating_culture INTEGER NOT NULL,  -- 文化真實性／導覽品質
    rating_access  INTEGER NOT NULL,  -- 交通與可及性
    rating_value   INTEGER NOT NULL,  -- 性價比
    rating_overall INTEGER NOT NULL,  -- 整體推薦度
    title          TEXT,
...
CREATE TABLE IF NOT EXISTS trip_groups (
    ...
    status         TEXT NOT NULL DEFAULT 'open', -- open/full/closed/completed/cancelled
    created_at     DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS trip_group_messages (
    ...
    member_id     INTEGER NOT NULL,
    content       TEXT NOT NULL,
    reply_to_id   INTEGER,          -- 回覆哪一則留言（NULL 代表是新的一則留言，不是回覆）
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS companion_ratings (
    ...
    rating_punctual      INTEGER NOT NULL, -- 準時可靠
    rating_communication INTEGER NOT NULL, -- 溝通互動
    rating_respect       INTEGER NOT NULL, -- 尊重與禮貌
    rating_overall       INTEGER NOT NULL, -- 整體體驗
    comment              TEXT,
    created_at           DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (trip_group_id) REFERENCES trip_groups(id),
